...
#  change_id_nos - Changes the 'id' column in a usage_compute_**** table to values required to append the rows to usage_compute_fix.
#
import sys
import os
import logging
...
#  Find the maximum current 'id' value in 'usage_compute_fix'.
...
print("next id: ",next_id_in_usage_compute)

#  Change the 'id' values into those required for appending to 'usage_compute_fix'.
#for idrow in range(1,49):
idchange = 419867
for idrow in range(start_tmp_id,end_tmp_id+1):
    print("next id in usage compute = ",next_id_in_usage_compute)
    sql = "UPDATE usage_compute_" + slurm_acct_no + " SET id = %s WHERE id = %s"
    data = (next_id_in_usage_compute,idrow,)
    print("Changing 'id' value from ",idrow," to ",next_id_in_usage_compute,".")
    results = []
    amiedb_call(sql,data,script_name,results)

    next_id_in_usage_compute = next_id_in_usage_compute + 1
#    sys.exit()
