...
sql = "SELECT project_id,person_id FROM local_info WHERE cluster = 'aces' ORDER BY project_id"
data = ()
results = []
...
#project_id_list = ['p.nairr240137.000']
...
#  Find all persons in the project.
    persons_in_project = []
    sql = "SELECT person_id FROM local_info WHERE project_id = %s and cluster = 'aces'"
    data = (project_id,)
    results = []
    amiedb_call(sql,data,script_name,results)
...
#  Extract a plethora of info from 'proj_stat'.
        sql = "SELECT first_name,last_name,email,access_id,uid,gid,pi,proj_stat,acct_stat,override FROM local_info WHERE project_id = %s AND person_id = %s and cluster = 'aces'"
...
#  Extract latest start and end dates from 'rpc_packets'.
        sql = "SELECT start_date,end_date FROM rpc_packets WHERE project_id = %s AND cluster = 'aces' order by trans_rec_id limit 1"
        data = (project_id,)
        results = []
        try:
            amiedb_call(sql,data,script_name,results)
...
#  Check if 'access_id' already exists in 'user_info'.
        ...
        if pi == 'Y':
            sql = "SELECT * FROM user_info_aces WHERE person_id = %s AND project_id = %s AND pi = 'Y'"
        else:
            sql = "SELECT * FROM user_info_aces WHERE person_id = %s AND project_id = %s AND pi = 'N'"
        data = (person_id,project_id,)
        results = []
        amiedb_call(sql,data,script_name,results)
...
#            print("ADDING")
#  sql = "SELECT first_name,last_name,email,access_id,uid,gid,pi,proj_stat,acct_stat,override FROM local_info WHERE project_id = %s AND person_id = %s and cluster = 'aces'"
            sql = "INSERT INTO user_info_aces (first_name,last_name,email,person_id,project_id,uid,gid,access_id,pi,academic_status,degree_field,degree,organization,country,start_date,end_date,proj_stat,acct_stat,override) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
