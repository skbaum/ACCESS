...
#  slurm_faster.py - Extracts faster ACCESS usage information from 'job_processing' and 'faster_job_table' for entries with valid Slurm IDs.
#
#  The general strategy is:
#
#  1. Extract unique ACCESS Slurm account (slurm_acct) numbers for faster accounts in 'local_info' table.
#  2. Extract last 'serial_no' - corresponding to 'job_table_record_id' in 'faster_job_processing' table - from 'usage_compute' table.
#  3. Extract all the 'job_table_record_id' values from the 'job_processing' table where 'state' = 1.
#  4. Perform an inner join of 'job_processing' and 'faster_job_table' on the first column serial numbers to extract
#     the info from 'faster_job_table' needed to construct an ACCESS usage report packet.
...
#    a. Check if the Slurm ID from 'faster_job_table' - i.e. the 'account' column' - matches an entry in the list of ACCESS Slurm IDs extracted from 'local_info'.
...
# Print no. of entries in usage_compute_fix for each Slurm account number.
with open('usage.txt', 'w') as f:
    for san in valid_slurm_account_numbers:
        sql = "select count(*) from usage_compute_fix where local_record_id = %s"
        data = (str(san),)
        results = []
        amiedb_call(sql,data,script_name,results)
        try:
...
#    sql = "select person_id from local_info where uid = %s and pi = 'Y' and cluster = 'faster' and proj_stat = 'active'"
    sql = "select distinct person_id,slurm_acct,uid from local_info where uid = %s and pi = 'Y' and cluster = 'faster' and proj_stat = 'active'"
    data = (vid,)
    results = []
    amiedb_call(sql,data,script_name,results)
    if len(results) != 0:
...
#  EXTRACT LIST OF VALID JOB ID NUMBERS FROM job_processing TABLE
try:
...
#  Create list of valid 'job_table_record_id' values in 'job_processing'.
valid_job_table_record_id_values = []
query = "select job_table_record_id from job_processing where state = '1' order by 'job_table_record_id'"
data = ()
cursor.execute(query,data)
for values in cursor:
    valid_job_table_record_id_values.append(values[0])
print("List from 'job_processing'':")
...
already_in_usage_compute_fix = 0
...
msg1 = "Log of faster jobs that can't be processed to send a usage packet.\n"
msg2 = " job index  slurm acct.   uid    SU\n"

#  Create a list of all ordered 'serial_no' values in 'usage_compute_fix'.
sql = "SELECT serial_no FROM usage_compute_fix ORDER BY serial_no"
data = ()
results = []
amiedb_call(sql,data,script_name,results)
...
print("List from 'usage_compute_fix':")
...
print("Purged list:")
purge_length = len(valid_indexes)
print("purge length = ", purge_length)
purge_min = valid_indexes[0]
print("purge min. = ",purge_min)
print("min = ",purge_min)
purge_max = valid_indexes[-1]
print("purge max. = ",purge_max)

#  Select the largest 'id' value in 'usage_compute_fix'.
...
#  PERFORM AN INNER JOIN BETWEEN faster_job_table AND job_processing TO CONNECT CHARGES WITH SLURM ACCOUNT NUMBERS
#  Perform the inner join extraction.
    ...
    try:
        ...
        print("valid_index = ",valid_index)
        query = "SELECT job_db_inx,account,from_unixtime(time_submit),from_unixtime(time_start),from_unixtime(time_end),update_time,service_units,nodes_alloc,num_cpus,id_job,mem_req,job_name,id_user,`partition` FROM faster_job_table INNER JOIN job_processing ON faster_job_table.job_db_inx = job_processing.job_table_record_id where job_db_inx = %s"
        data = (valid_index,)
        cursor.execute(query,data)
...
#  CHECK IF SLURM ACCOUNT NUMBER IS AN INTEGER
    ...
    print("****************** account = ",account," null = ",null)
...
# -----------------------------------------------
    ...
    status = 'N'
#  Check if 'job_processing' serial number is already in 'usage_compute_fix' table.  If so, skip to next line.
    query4 = "SELECT * FROM usage_compute_fix WHERE serial_no = %s"
    data4 = (job_db_inx,)
    results = []
...
#    print("results = ",results)
    if len(results) > 0:
        print("The 'job_processing' serial no. ",job_db_inx," is already in 'usage_compute_fix'.  Skipping to next line.")
        print("---------------------------------------------------------------------")
        already_in_usage_compute_fix = already_in_usage_compute_fix + 1
        continue
...
#  Insert an entry into 'usage_compute_fix'.
    if test == 0:
        query3 = """INSERT INTO usage_compute_fix (id,serial_no,person_id,project_id,uid,local_record_id,resource,submit_time,start_time,end_time,charge,node_count,cpu_core_count,job_name,max_memory,queue,parent_record_id,local_reference,time_stored,access_id,status) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"""
        print("$$$$$$$ VALUES TO INSERT INTO usage_compute_fix $$$$$")
        ...
        data3 = (usage_id,valid_index,person_id,project_id,uid,slurm_acct,cluster,time_submit,time_start,time_end,service_units,int(nodes_alloc),int(num_cpus),id_job,mem_req,partition,parent_record_id,local_reference,tstamp(),access_id,status)
...
#            amiedb_call(query3,data3,script_name,results)
            print("Successful insertion of usage report no. ",valid_index," into 'usage_compute_fix' for ",person_id," on project ",project_id)
            usage_id = usage_id + 1
        except Exception as e:
            msg1 = "Problem inserting serial no. " + valid_index + " into 'usage_compute_fix' for user " + person_id + " on project " + project_id + " with ACCESS ID " + access_id + ". Skipping to next serial number."
...
#        conn.close()
        ...
        print("TEST:  Not inserting serial no. ",valid_index," entry into 'usage_compute_fix'.")
...
print("Entries already in 'usage_compute_fix': ",already_in_usage_compute_fix)
...
print(" ")
print("No. of packets processed: ",purge_length)
