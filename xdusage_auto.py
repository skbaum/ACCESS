...
#parser.add_argument("-s","--slurm_no", help="Enter the Slurm account number for the project.")
...
#  Check for a help message request.
    if sys.argv[1] in help_list:
        print(" ")
        print(" Program: ",script_name," - returns counts, usage, or job numbers from ACCESS")
        ...
        if len(sys.argv) < 4:
            print("Specify a type ('usage', etc), a cluster name ('aces'), a slurm account number ('155485226413'). Try again.")
            sys.exit()
        else:
            itype = sys.argv[1]
            resource_name = sys.argv[2]
            slurm_no = sys.argv[3]
            if resource_name not in cluster_names:
...
#  Retrieve project_id and grant_number from 'local_info'.
sql = "SELECT project_id,grant_number FROM local_info WHERE slurm_acct = %s"
data = (slurm_no,)
results = []
amiedb_call(sql,data,script_name,results)
...
#  Find resource_name from cluster name.
...
#    print("x_url_full = ",x_url_full)
    response = requests.get(x_url_full, headers=x_headers)
    acc_usage = response.json()['result'][0]['su_used']
    print(acc_usage)
else:
...
#  Find jobs report numbers sent by TAMU but not received by ACCESS.
    print("Start list comparison.")
    unreceived_serial_nos = [val for val in tamu_local_job_id if val not in access_local_job_id]
    no_of_missing_reports = len(unreceived_serial_nos)
    print("Number of missing reports: ",no_of_missing_reports)
    print("End list comparison.")
    print("Start set comparison.")
    diff_local_access = set(tamu_local_job_id) - set(access_local_job_id)
    print("End set comparison.")
...
#        print("results = ",results)
    ...
    if update == 'Y':
        unrec_ids = unreceived_ids[0:400]
        print("Processing ",len(unrec_ids)," of ",len(unreceived_ids)," total unreceived reports.")
#        sys.exit()
        for ids in unrec_ids:
            if resource_name == 'aces':
